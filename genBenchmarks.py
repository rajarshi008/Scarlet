			...
			for size in sample_sizes:
				for length_range in trace_lengths:
						...
						if trace_type == 'words':
							...
							convertFileType(wordfile=word_file, tracefile=trace_file, operators=['F', 'X', '&'])
						else:
							sample.generator(formula=formula, length_range=length_range, num_traces=size, filename=trace_file, is_word=(trace_type=='words'), operators=operators)
...
def main():
	...
	parser.add_argument('--size', dest='sample_sizes', default=[5], nargs='+', type=int)
	parser.add_argument('--lengths', dest='traceLengths', default=[['7', '9']], nargs='+', type=list)
	...
	trace_lengths = list([(int(r[0]), int(r[1])) for r in args.traceLengths])
